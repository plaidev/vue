	...
	Watcher.prototype.set = function (value) {
	  ...
	  if (forContext && forContext.alias === this.expression) {
	    if (forContext.filters) {
	      ("development") !== 'production' && _utilIndex.warn('It seems you are using two-way binding on ' + 'a krt-for alias (' + this.expression + '), and the ' + 'krt-for has filters. This will not work properly. ' + 'Either remove the filters or use an array of ' + 'objects and bind to object properties instead.', this.vm);
	      return;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function checkTerminalDirectives(el, attrs, options) {
	  // skip krt-pre
	  if (_utilIndex.getAttr(el, 'krt-pre') !== null) {
	    return skip;
	  }
	    ...
	    if (tokens) {
	      ...
	      if (true) {
	        if (name === 'class' && Array.prototype.some.call(attrs, function (attr) {
	          ...
	          _utilIndex.warn('class="' + rawValue + '": Do not mix mustache interpolation ' + 'and krt-bind for "class" on the same element. Use one or the other.', options);
	        }
	      }
	    } else
	...
	exports['default'] = {
	  ...
	  bind: function bind() {
	    ...
	    if (this.el.nodeType === 8) {
	      ...
	      this.anchor = _utilIndex.createAnchor('krt-html');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var vFor = {
	  ...
	  bind: function bind() {
	    ...
	    if (!this.alias) {
	      ("development") !== 'production' && _utilIndex.warn('Invalid krt-for expression "' + this.descriptor.raw + '": ' + 'alias is required.', this.vm);
	      return;
	    }
	    ...
	    // setup anchor nodes
	    this.start = _utilIndex.createAnchor('krt-for-start');
	    this.end = _utilIndex.createAnchor('krt-for-end');
	...
	if (true) {
	  vFor.warnDuplicate = function (value) {
	    _utilIndex.warn('Duplicate value found in krt-for="' + this.descriptor.raw + '": ' + JSON.stringify(value) + '. Use track-by="$index" if ' + 'you are expecting duplicate values.', this.vm);
	  };
	}
	...
	exports['default'] = {
	  ...
	  bind: function bind() {
	    ...
	    if (!el.__vue__) {
	      ...
	      if (next && _utilIndex.getAttr(next, 'krt-else') !== null) {
	        _utilIndex.remove(next);
	        this.elseEl = next;
	      }
	      // check main block
	      this.anchor = _utilIndex.createAnchor('krt-if');
	      _utilIndex.replace(el, this.anchor);
	    } else {
	      ("development") !== 'production' && _utilIndex.warn('krt-if="' + this.expression + '" cannot be ' + 'used on an instance root element.', this.vm);
	      this.invalid = true;
	    }
	  },
	...
	exports['default'] = {
	  ...
	  bind: function bind() {
	    ...
	    if (this.hasRead && !this.hasWrite) {
	      ("development") !== 'production' && _utilIndex.warn('It seems you are using a read-only filter with ' + 'krt-model="' + this.descriptor.raw + '". ' + 'You might want to use a two-way filter to ensure correct behavior.', this.vm);
	    }
	    var el = this.el;
	    var tag = el.tagName;
	    var handler;
	    if (tag === 'INPUT') {
	      ...
	      ("development") !== 'production' && _utilIndex.warn('krt-model does not support element type: ' + tag, this.vm);
	      return;
	    }
	    el.__v_model = this;
	...
	exports['default'] = {
	  ...
	  update: function update(handler) {
	    ...
	    if (typeof handler !== 'function') {
	      ("development") !== 'production' && _utilIndex.warn('krt-on:' + this.arg + '="' + this.expression + '" expects a function value, ' + 'got ' + handler, this.vm);
	      return;
	    }
	  ...
	  bind: function bind() {
	    ...
	    if (tokens) {
	      ...
	      if (true) {
	        ...
	        // warn src
	        if (attr === 'src') {
	          _utilIndex.warn(raw + 'interpolation in "src" attribute will cause ' + 'a 404 request. Use krt-bind:src instead.', this.vm);
	        }
	
	        // warn style
	        if (attr === 'style') {
	          _utilIndex.warn(raw + 'interpolation in "style" attribute will cause ' + 'the attribute to be discarded in Internet Explorer. ' + 'Use krt-bind:style instead.', this.vm);
	        }
	      }
	    }
	  },
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = {
	  bind: function bind() {
	    ("development") !== 'production' && _utilIndex.warn('krt-ref:' + this.arg + ' must be used on a child ' + 'component. Found on <' + this.el.tagName.toLowerCase() + '>.', this.vm);
	  }
	};
	module.exports = exports['default'];
	...
	exports['default'] = {
	  ...
	  cleanup: function cleanup(value) {
	    ...
	    while (i--) {
	      ...
	      var keys = _utilIndex.isPlainObject(key) ? Object.keys(key) : [key];
	...
	exports['default'] = {
	  ...
	  bind: function bind() {
	    if (!this.el.__vue__) {
	      ...
	      // create a ref anchor
	      this.anchor = _utilIndex.createAnchor('krt-component');
	      _utilIndex.replace(this.el, this.anchor);
	      // remove is attribute.
	      ...
	      if (this.descriptor.ref) {
	        this.el.removeAttribute('krt-ref:' + _utilIndex.hyphenate(this.descriptor.ref));
	      }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function transclude(el, options) {
	  ...
	  if (_utilIndex.isFragment(el)) {
	    ...
	    _utilIndex.prepend(_utilIndex.createAnchor('krt-start', true), el);
	    el.appendChild(_utilIndex.createAnchor('krt-end', true));
	  }
	  return el;
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function extractFragment(nodes, parent) {
	  ...
	  for (var i = 0, l = nodes.length; i < l; i++) {
	    var node = nodes[i];
	    if (_utilIndex.isTemplate(node) && !node.hasAttribute('krt-if') && !node.hasAttribute('krt-for')) {
	    ...
	    for (var i = 0, l = attrs.length; i < l; i++) {
	      ...
	      if (eventRE.test(name)) {
	        ...
	        if (typeof handler === 'function') {
	          ...
	          _utilIndex.warn('krt-on:' + name + '="' + attrs[i].value + '" ' + 'expects a function value, got ' + handler, vm);
	        }
	      }
	    }
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = function (Vue) {
	  ...
	  Vue.prototype._compile = function (el) {
	    ...
	    if (el.nodeType === 1 && _utilIndex.getAttr(el, 'krt-pre') !== null) {
	      return;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = {
	  ...
	  bind: function bind() {
	    this.anchor = _utilIndex.createAnchor('krt-partial');
	    _utilIndex.replace(this.el, this.anchor);
