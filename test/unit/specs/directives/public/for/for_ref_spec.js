...
describe('krt-for + ref', function () {
  ...
  it('normal', function (done) {
    var vm = new Vue({
      ...
      template: '<test krt-for="item in items" :item="item" krt-ref:test></test>',
      components: {
        test: {
          props: ['item']
        }
  ...
  it('nested', function () {
    var vm = new Vue({
      el: el,
      template: '<c1 krt-ref:c1></c1>',
      components: {
        c1: {
          template: '<div krt-for="n in 2" krt-ref:c2></div>'
        }
      }
    })
